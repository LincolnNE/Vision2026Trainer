...
class WordBasedImageScraper:
    ...
    def search_images_for_word(self, word: str, max_images: int = 5) -> List[str]:
        """
        특정 단어에 대한 이미지를 검색합니다.
        ...
        """
        logger.info(f"단어 '{word}'에 대한 이미지 검색 시작...")
        
        image_urls = []
        
        # Unsplash API를 사용한 이미지 검색
        try:
            unsplash_urls = self._search_unsplash(word, max_images)
            image_urls.extend(unsplash_urls)
        except Exception as e:
            logger.warning(f"Unsplash 검색 실패 ({word}): {e}")
        
        # 추가 검색 소스들
        for source_name, base_url in self.search_sources.items():
            if source_name != 'unsplash':  # Unsplash는 이미 처리됨
                try:
                    urls = self._search_generic(word, base_url, max_images)
                    image_urls.extend(urls)
                except Exception as e:
                    logger.warning(f"{source_name} 검색 실패 ({word}): {e}")
    ...
    def _search_unsplash(self, word: str, max_images: int) -> List[str]:
        """Unsplash에서 이미지를 검색합니다."""
        try:
            # Unsplash API 사용 (무료 버전)
            access_key = "your_unsplash_access_key"  # 실제 사용 시 API 키 필요
            ...
            # API 키가 없는 경우 더미 URL 생성
            if access_key == "your_unsplash_access_key":
                return self._generate_dummy_urls(word, max_images)
            ...
            logger.warning(f"Unsplash API 검색 실패: {e}")
            return self._generate_dummy_urls(word, max_images)
    
    def _search_generic(self, word: str, base_url: str, max_images: int) -> List[str]:
        """일반적인 웹사이트에서 이미지를 검색합니다."""
        try:
            search_url = f"{base_url}{quote(word)}"
            ...
            logger.warning(f"일반 검색 실패 ({base_url}): {e}")
            return []
    ...
    def _generate_dummy_urls(self, word: str, max_images: int) -> List[str]:
        ...
        # Unsplash의 무료 이미지 서비스 사용
        base_urls = [
            f"https://source.unsplash.com/400x400/?{quote(word)}",
        ...
        for i in range(max_images):
            if i < len(base_urls):
                dummy_urls.append(base_urls[i])
            else:
                # 추가 URL 생성
                dummy_urls.append(f"https://source.unsplash.com/400x400/?{quote(word)}&sig={i}")
    ...
    def scrape_images_for_categories(self, categorized_words: Dict[str, List[str]], 
        ...
        for category, words in categorized_words.items():
            ...
            for word in selected_words:
                try:
                    word_urls = self.search_images_for_word(word, max_images_per_word)
                    ...
                    logger.warning(f"단어 '{word}' 이미지 검색 실패: {e}")
                    continue
...
class WordBasedImageDataset(Dataset):
    """단어 기반 이미지 데이터셋 클래스"""
...
class WordBasedPipeline:
    """단어 기반 자동 스크래핑 이미지 분류 파이프라인 메인 클래스"""
    ...
    def __init__(self, data_dir: str = "./dataset", model_dir: str = "./models", results_dir: str = "./results"):
        ...
        # 스크래퍼 초기화
        self.scraper = WordBasedImageScraper()
    ...
    def load_words_and_scrape_images(self, csv_files: List[str], max_images_per_word: int = 3) -> Tuple[List[str], List[str]]:
        """
        CSV 파일에서 단어를 로딩하고 이미지를 스크래핑합니다.
        ...
        """
        logger.info("단어 로딩 및 이미지 스크래핑 시작...")
    ...
    def _create_dummy_data(self) -> Tuple[List[str], List[str]]:
        ...
        dummy_data = {
            "nature": [
                ...
                "https://source.unsplash.com/400x400/?forest",
...
def main():
    """메인 실행 함수"""
    logger.info("단어 기반 자동 스크래핑 이미지 분류 파이프라인 시작")
    
    # 파이프라인 초기화
    pipeline = WordBasedPipeline()
    
    try:
        # 1. CSV 파일에서 단어 로딩 및 이미지 스크래핑
